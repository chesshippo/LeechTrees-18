...
struct Search {
    ...
    bool use_structural_bounds=false;
    bool use_late_t9a=false;
    bool stop_at_first=false;
    int root_branch=-1;
    int root_valid_branches=0;
    vector<int> branch_path;
    int branch_path_base_depth=4;
    int root_branch_depth=4;
    array<int,18> valid_children_max{};
    int stop_edges=-1;
    int attached_equality_r=-1;
    int separate_equality_r=-1;
    bool use_parity_coherence=false;
    bool shadow_parity_coherence=false;
    multi_edge_parity_coherence::Config parity_coherence_config;
    multi_edge_parity_coherence::Counters parity_coherence_counters;
    multi_edge_parity_coherence::Checker parity_coherence_checker;
    long long parity_coherence_shadow_reject=0;
    ...
    bool rec() {
        ...
        if (use_structural_bounds) {
            int failed=structural_cut_bounds(z);
        ...
        // Run the parity-coherent block precheck after the legacy block
        // layer.  When both are enabled this avoids repeating its port scan
        // on prefixes the legacy layer already rejects.  The ordering changes
        // performance only: both checks are independent necessary conditions.
        if (use_parity_coherence || shadow_parity_coherence) {
            auto outcome=parity_coherence_checker.check(
                multi_cover_input(z),parity_coherence_config,
                parity_coherence_counters);
            if (!outcome.possible) {
                if (shadow_parity_coherence)
                    parity_coherence_shadow_reject++;
                else return false;
            }
        }
        ...
        for (auto q:cand) {
            ...
            if ((int)edges.size()==root_branch_depth) {
    ...
    void print_result(const string& mode) const {
        ...
        if (use_parity_coherence || shadow_parity_coherence) {
            const auto& c=parity_coherence_counters;
            cout<<" parity_coherence="
                <<(use_parity_coherence?"on":"shadow")
                <<" pc_checks="<<c.checks
                <<" pc_skipped="<<c.skipped
                <<" pc_pairs="<<c.component_pairs
                <<" pc_patterns="<<c.port_patterns
                <<" pc_lengths="<<c.candidate_lengths
                <<" pc_no_relation="<<c.no_relation_fail
                <<" pc_profiles="<<c.profiles_tested
                <<" pc_mass_profiles="<<c.mass_profiles_tested
                <<" pc_no_profile="<<c.no_profile_fail
                <<" pc_shadow_reject="<<parity_coherence_shadow_reject;
        }
        if (use_multi_edge_cover || shadow_multi_edge_cover) {
...
int main(int argc,char**argv) {
    ...
    vector<int> branch_path;
    bool use_parity_coherence=false,shadow_parity_coherence=false;
    multi_edge_parity_coherence::Config parity_coherence_config;
    bool use_structural_bounds=false,use_late_t9a=false;
    ...
    for (int i=1;i<argc;i++) {
        ...
        if (s=="--mode" && i+1<argc) mode=argv[++i];
        ...
        else if (s=="--multi-edge-parity-coherence")
            use_parity_coherence=true;
        else if (s=="--multi-edge-parity-coherence-shadow")
            shadow_parity_coherence=true;
        else if (s=="--multi-edge-parity-max-components" && i+1<argc)
            parity_coherence_config.max_components=stoi(argv[++i]);
        else if (s=="--structural-cut-bounds") use_structural_bounds=true;
        else if (s=="--late-t9a-profile") use_late_t9a=true;
            ...
            cout<<"Usage: a2_topology_free_search "
                  ...
                  "a2_separate_equality|g001_row1|g001_row7|small] "
                  ...
                  "[--structural-cut-bounds] [--late-t9a-profile] "
    ...
    if (use_parity_coherence && shadow_parity_coherence) {
        cerr<<"choose either active or shadow parity coherence, not both\n";
        return 64;
    }
    ...
    if (parity_coherence_config.max_components<1 ||
        parity_coherence_config.max_components>18) {
        cerr<<"parity-coherence component threshold must be in 1..18\n";
        return 64;
    }
    ...
    if (mode=="small") {
        ...
        for (int n=2;n<=6;n++) {
            Search s(n);
            s.use_parity_coherence=use_parity_coherence;
            s.shadow_parity_coherence=shadow_parity_coherence;
            s.parity_coherence_config=parity_coherence_config;
            s.use_structural_bounds=use_structural_bounds;
            s.use_late_t9a=use_late_t9a;
    ...
    int partition_depth=(mode=="a2_attached_equality"?6:
        mode=="a2_separate_equality"?5:
        (mode=="g001_row1" || mode=="g001_row7")?3:4);
    s.branch_path_base_depth=partition_depth;
    s.root_branch_depth=partition_depth;
    ...
    if (mode=="g001_row1") {
        // Zero-based G001 row 1: e1 and e2 are adjacent, and e4 meets e1
        // but not e2.  Up to weighted-tree isomorphism this is the path
        //     0 --4-- 1 --1-- 2 --2-- 3.
        // Its internal distances are {1,2,3,4,5,7}, so T2a forces the next
        // physical edge weight to be 6; weights 5 and 7 are nonphysical.
        s.add_edge(1,2,1); s.add_edge(2,3,2); s.add_edge(0,1,4);
        s.rec();
    } else if (mode=="g001_row7") {
        // Zero-based G001 row 7: e1 and e2 are disjoint, while e3 meets
        // both.  Up to weighted-tree isomorphism this is the path
        //     0 --1-- 1 --3-- 2 --2-- 3.
        // Its six internal distances are exactly 1,...,6, so T2a forces
        // the next physical edge weight to be 7.
        s.add_edge(0,1,1); s.add_edge(2,3,2); s.add_edge(1,2,3);
        s.rec();
    } else if (mode=="a2_attached_equality") {
    ...
    if (mode!="a2_attached_equality" && mode!="a2_separate_equality" &&
        mode!="a2_separate" && mode!="g001_row1" && mode!="g001_row7" &&
