...
foreach ($key in $expectedKeys) {
    ...
    $workerPath = $stdoutPath -replace '\.stdout\.txt$', '.worker.ps1'
    foreach ($artifact in @(
        $stdoutPath,$stderrPath,$pidPath,$donePath,$workerPath)) {
    ...
    $quotedArguments = ($argumentTokens | ForEach-Object {
        "'" + ($_ -replace "'", "''") + "'"
    }) -join ','
    $expectedWorkerLines = @(
        '$ErrorActionPreference=''Stop''',
        ...
        '$exitCode=-999',
        'try {',
        "    & '$solver' @($quotedArguments) 1> '$stdoutPath' 2> '$stderrPath'",
    ...
    Assert-ExactOrderedStrings @(Get-Content -LiteralPath $workerPath) `
        $expectedWorkerLines "$key worker script"
