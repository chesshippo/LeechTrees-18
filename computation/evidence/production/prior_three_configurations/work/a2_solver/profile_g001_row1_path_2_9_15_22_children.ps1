param(
    ...
    [ValidateRange(0,30)] [int] $FirstChild = 0,
    [ValidateRange(0,30)] [int] $LastChild = 30,
    [switch] $DryRun,
    [switch] $AggregateOnly
)

# Profiles the exact 31-way split below row-1 path 2,9,15,22 at depth 12.
# Examples:
#   .\profile_g001_row1_path_2_9_15_22_children.ps1 `
...
$targetBaselineFrontier = [int64]489707
$targetCoverFrontier = [int64]277927
$profileColumns = @(
...
function Get-ChildStem {
    param([int] $Child)
    return "$ProfileSet.path_2_9_15_22_$($Child.ToString('00'))"
}
...
if ($AggregateOnly) {
    ...
    $expectedCsvPaths = @(0..30 | ForEach-Object {
        (Get-ChildArtifacts $_).csv
    })
    $actualCsvPaths = @(Get-ChildItem -LiteralPath $runDir -File |
        Where-Object {
            $_.Name -like "$ProfileSet.path_2_9_15_22_*.profile.csv"
    ...
    if ($missing.Count -or $unexpected.Count -or
        $actualCsvPaths.Count -ne 31) {
        Write-Output "EXPECTED_CHILD_CSV=31 RECORDED=$($actualCsvPaths.Count)"
        if ($missing.Count) {
        ...
        throw 'The 31-child profile set is incomplete or has extra CSV files.'
    }
    ...
    foreach ($child in 0..30) {
        ...
        for ($index = 0; $index -lt 2; ++$index) {
            ...
            if ($row.profile_set -cne $ProfileSet -or
                ...
                $row.branch_path -cne "2,9,15,22,$child" -or
    ...
    Write-Output "CHILDREN=31 BASELINE_NODES_DIAGNOSTIC_SUM=$baselineNodes COVER_NODES_DIAGNOSTIC_SUM=$coverNodes NODE_SUMS_ADDITIVE=NO"
    ...
    if ($baselineFrontier -ne $targetBaselineFrontier -or
        ...
        throw 'The 31 child frontiers do not reproduce path 2,9,15,22.'
    }
    Write-Output 'G001_ROW1_PATH_2_9_15_22_PROFILE_SUM_OK'
    return
}
...
Write-Output "Run -ProfileSet $ProfileSet -AggregateOnly after all 31 children finish."
